...
- Если группа кейсов включает 3+ связанных подгруппы — создай общую
  родительскую папку. Например:
    "Создание заявки"
      "Загрузка CSV"          ← подгруппа
      "Предварительный отчёт"  ← подгруппа
      "Валидация"             ← подгруппа
- Название корневой секции должно отражать название фичи.
...
- targetSectionPath — массив названий от корня до целевой секции.
  Например: ["Массовая корректировка", "Запуск корректировки", "Успешная обработка"]
...
Пример правильной иерархии:
  ...
  ["Массовая корректировка", "Запуск корректировки", "Успешная обработка"]
  ["Массовая корректировка", "Запуск корректировки", "Ошибка обработки"]
...
Верни только JSON без markdown-обёртки:
{
  "cases": [
    ...
    },
    {
      "id": "TC-002",
      "title": "...",
      "targetSectionPath": ["Массовая корректировка", "Запуск корректировки", "Успешная обработка"]
    }
  ]
}`,
        },
