...
`Sidebar` поддерживает собственный SSE для каждой активной фичи:

- **SSE handler**: реагирует на `stage-update`, `completed`, `failed`, `blocked`, `waiting_for_qa` → `loadPipeline(slug)`
- **Polling**: опрос каждые 10с для статусов `running`, `blocked`, `waiting_for_qa`
- **Auto-reconnect**: exponential backoff, max 30с
...
- **addEventListener**: ловит все типизированные события NestJS SSE; `type` берётся из `event:` заголовка, не из `data`
- **onmessage**: fallback для событий без типа (`type='unknown'`)
...
- **scheduleRefresh** вызывается на: `stage-update`, `progress`, `blocked`, `waiting_for_qa`, `completed`, `failed`, `fill-gaps-started`, `fill-gaps-done`, `log "Completed"`
- **fill-gaps визуал**: Test Cases = running (спиннер), Coverage = waiting (точки); бейдж статуса обновляется сразу при `fill-gaps-started`
- **Backend порядок**: `handleStageResult` (сохраняет `stageResults` в БД) выполняется ДО SSE событий — при HTTP poll данные уже актуальны
- **pipeline:stage-update** эмитится для каждого статуса этапа: `running`, `success`, `blocked`, `waiting_for_qa` — гарантирует, что UI обновится без F5
